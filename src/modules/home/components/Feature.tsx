...
const FeatureCard = ({ children }: { children: React.ReactNode }) => {
  return (
    <Link href="/builder" passHref={true}>
      <div
        className={`transition ease-in-out delay-100 duration-300 bg-resume-50 hover:bg-resume-100 text-resume-800
      hover:text-resume-900 hover:scale-105 fill-resume-800 px-6 py-10 lg:p-12 flex shadow-md cursor-pointer relative rounded-xl`}
      >
        {children}
      </div>
    </Link>
...
const CardPinnnedIcon = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="backdrop-blur-2xl bg-resume-50 rounded-full p-2 shadow-level-hard absolute right-0 top-0 -mt-4 -mr-1">
      {children}
    </div>
  );
};
